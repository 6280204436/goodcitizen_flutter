...
class ProfileController extends GetxController {
  ...
  @override
  void onReady() {
    userResponseModel.value = Get.arguments["model"];
    useremail.value = userResponseModel?.value?.data?.email ?? "";
    name.value =
        "${userResponseModel?.value?.data?.firstname ?? ""} ${userResponseModel?.value?.data?.lastname ?? ""}" ??
            "";

    confimPasswordController.text =
        userResponseModel.value?.data?.hospitalDoc ?? "";
    if (userResponseModel?.value?.data?.loyaltypoint != null) {
      loyaltyPoints.value =
          userResponseModel?.value?.data?.loyaltypoint.toString() ?? "0";
    }
  ...
  void logout() {
    repository.logoutApiCall().then((value) {
      if (value != null) {
        ...
        nativeIOSService.stopLocationEmit();
